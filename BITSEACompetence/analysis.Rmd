...
# plot
p <- data %>%
  dplyr::select(id,group, pre_bitseacomp_mean, post_bitseacomp_mean, fu_bitseacomp_mean) %>%
...
pvalue>0.05. We do not have evidence that there is a difference in the change over from Baseline to Post amongst the groups, such that PCITT showed the largest change, then COSP then Waitlist.
...
There is greater change in PCITT(-.138) than in COSP (-.115).
