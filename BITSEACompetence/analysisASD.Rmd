---
title: "PCIT-CoS-Waitlist RCT Paper 1: BITSEA Competence, Controlling for ASD"
author: "Nancy Briggs"
date: "05 JAN 2021"
output:
  html_document:
...
### BITSEA Competence Adjusting for ASD {.tabset .tabset-fade .tabset-pills}
...
desctable <- data %>%
  ...
  mutate(score1 = case_when( 
    pre_bitseacomp_mean == -9 ~ NA_real_,          #assign missing values to zeros
    TRUE ~ pre_bitseacomp_mean)) %>%
  mutate(score2 = case_when( 
    post_bitseacomp_mean == -9 ~ NA_real_,         #assign missing values to zeros
    TRUE ~ post_bitseacomp_mean)) %>%
  mutate(score3 = case_when( 
    fu_bitseacomp_mean == -9 ~ NA_real_,           #assign missing values to zeros
    TRUE ~ fu_bitseacomp_mean)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(pre_bitseacomp_mean, na.rm = T ),
            sd1=sd(pre_bitseacomp_mean, na.rm = T ),
            median1=median(pre_bitseacomp_mean, na.rm = T ),
            min1=min(pre_bitseacomp_mean, na.rm = T ),
            max1=max(pre_bitseacomp_mean, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(post_bitseacomp_mean, na.rm = T ),
            sd2=sd(post_bitseacomp_mean, na.rm = T ),
            median2=median(post_bitseacomp_mean, na.rm = T ),
            min2=min(post_bitseacomp_mean, na.rm = T ),
            max2=max(post_bitseacomp_mean, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(fu_bitseacomp_mean, na.rm = T ),
            sd3=sd(fu_bitseacomp_mean, na.rm = T ),
            median3=median(fu_bitseacomp_mean, na.rm = T ),
            min3=min(fu_bitseacomp_mean, na.rm = T ),
            max3=max(fu_bitseacomp_mean, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,asdcase, group, pre_bitseacomp_mean, post_bitseacomp_mean, fu_bitseacomp_mean) %>%
  pivot_longer( cols = c("pre_bitseacomp_mean", "post_bitseacomp_mean", "fu_bitseacomp_mean"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_bitseacomp_mean",  1,
                ifelse(t == "post_bitseacomp_mean", 2, 3))) 
...
Ordering of effects are in the hypothesized, order: PCITT>COSP>Waitlist
