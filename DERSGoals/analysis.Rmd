---
title: "PCIT-CoS-Waitlist RCT Paper 1: DERS Goals"
author: "Nancy Briggs"
date: "`r Sys.Date()`"
output:
  html_document:
...
sum of DERSI13, DERSI18,-1*DERSI20, DERSI26, DERSI33
...
data <- data %>%
  dplyr::select(id:treatment_time_mins, dobchild, sexchild, numbhousehold, incfamily,
                ...
                #DERS Goal
                pre_ders_goal, post_ders_goal, fu_ders_goal
                )
...
desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(pre_ders_goal)) , 
            mean1=mean(pre_ders_goal, na.rm = T ),
            sd1=sd(pre_ders_goal, na.rm = T ),
            median1=median(pre_ders_goal, na.rm = T ),
            min1=min(pre_ders_goal, na.rm = T ),
            max1=max(pre_ders_goal, na.rm = T ),
            
            n2 = sum(!is.na(post_ders_goal)) , 
            mean2=mean(post_ders_goal, na.rm = T ),
            sd2=sd(post_ders_goal, na.rm = T ),
            median2=median(post_ders_goal, na.rm = T ),
            min2=min(post_ders_goal, na.rm = T ),
            max2=max(post_ders_goal, na.rm = T ),
            
            n3 = sum(!is.na(fu_ders_goal)) , 
            mean3=mean(fu_ders_goal, na.rm = T ),
            sd3=sd(fu_ders_goal, na.rm = T ),
            median3=median(fu_ders_goal, na.rm = T ),
            min3=min(fu_ders_goal, na.rm = T ),
            max3=max(fu_ders_goal, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group, pre_ders_goal, post_ders_goal, fu_ders_goal) %>%
  pivot_longer( cols = c("pre_ders_goal", "post_ders_goal", "fu_ders_goal"),  names_prefix = "ders_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ders_goal",  1,
                ifelse(t == "post_ders_goal", 2, 3))) 
...
dat <- data %>%
  dplyr::select(id,group,pre_ders_goal, post_ders_goal, fu_ders_goal) %>%
  pivot_longer( cols = c("pre_ders_goal", "post_ders_goal", "fu_ders_goal"),  names_prefix = "bitseaprob_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ders_goal",  1,
                ifelse(t == "post_ders_goal", 2, 3)))
