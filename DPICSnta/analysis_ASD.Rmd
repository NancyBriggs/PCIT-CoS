---
title: "PCIT-CoS-Waitlist RCT Paper 1: DPICS NTA, Controlling for ASD"
author: "Nancy Briggs"
date: "31 JAN 2021"
output:
  html_document:
...
### DPICS NTA Controlling for ASD {.tabset .tabset-fade .tabset-pills}
...
result0_5<-glmer(cbind(nta, verbal-nta) ~ asdcase + (1 | id), data=p, family=binomial)
result1<-glmer(cbind(nta, verbal-nta) ~ asdcase + group + (1 | id), data=p, family=binomial)
result2<-glmer(cbind(nta, verbal-nta) ~ asdcase + group + time + (1 | id), data=p, family=binomial)
result3<-glmer(cbind(nta, verbal-nta) ~ asdcase + group + time +group*time + (1 | id), data=p, family=binomial)
```

The model with asdcase*group*time does not converge. We will leave it out.

```{r}
result4<-glmer(cbind(nta, verbal-nta) ~ asdcase*group*time + (1 | id), data=p, family=binomial)
```
