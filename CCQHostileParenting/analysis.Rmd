---
title: "PCIT-CoS-Waitlist RCT Paper 1: CCQ Hostile Parenting"
author: "Nancy Briggs"
date: "18 March 2024"
output:
  html_document:
    ...
    toc_float: true
---
NO BOOTSTRAPPING OF EFFECT SIZE DONE

Outcomes:  pre_ccqc_sum, post_ccqc_sum, fu_ccqc_sum

### CCQ Hostile Parenting  {.tabset .tabset-fade .tabset-pills}
...
desctable <- data %>%
  ...
  mutate(score1 = case_when( 
    pre_ccqc_sum == -9 ~ NA_real_,          #assign missing values to zeros
    TRUE ~ pre_ccqc_sum)) %>%
  mutate(score2 = case_when( 
    post_ccqc_sum == -9 ~ NA_real_,         #assign missing values to zeros
    TRUE ~ post_ccqc_sum)) %>%
  mutate(score3 = case_when( 
    fu_ccqc_sum == -9 ~ NA_real_,           #assign missing values to zeros
    TRUE ~ fu_ccqc_sum)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(pre_ccqc_sum, na.rm = T ),
            sd1=sd(pre_ccqc_sum, na.rm = T ),
            median1=median(pre_ccqc_sum, na.rm = T ),
            min1=min(pre_ccqc_sum, na.rm = T ),
            max1=max(pre_ccqc_sum, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(post_ccqc_sum, na.rm = T ),
            sd2=sd(post_ccqc_sum, na.rm = T ),
            median2=median(post_ccqc_sum, na.rm = T ),
            min2=min(post_ccqc_sum, na.rm = T ),
            max2=max(post_ccqc_sum, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(fu_ccqc_sum, na.rm = T ),
            sd3=sd(fu_ccqc_sum, na.rm = T ),
            median3=median(fu_ccqc_sum, na.rm = T ),
            min3=min(fu_ccqc_sum, na.rm = T ),
            max3=max(fu_ccqc_sum, na.rm = T )  ) %>%
...
```{r, echo=FALSE}
#p#  pre_ccqc_sum, post_ccqc_sum, fu_ccqc_sum
# Histograms
# plot
p <- data %>%
  dplyr::select(id,group, pre_ccqc_sum, post_ccqc_sum ,fu_ccqc_sum) %>%
  pivot_longer( cols = c("pre_ccqc_sum", "post_ccqc_sum", "fu_ccqc_sum"),  names_prefix = "ccqc_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ccqc_sum",  1,
                ifelse(t == "post_ccqc_sum", 2, 3))) 
  

  
hist<-ggplot(p, aes(x=outcome) )+
  geom_histogram(alpha=0.6, binwidth = 1) +
  xlab("CCQ Hostile Parenting") +
  ylab("Frequency") +
...
Ordering is not in hypothesized order 

Mean Waitlist > mean PCITT > mean COSP 
...
pvalue>0.05. We fail to reject Hypothesis 1.
