---
title: "PCIT-CoS-Waitlist RCT Paper 1: CCQ Hostile Parenting, Controlling for ASD"
author: "Nancy Briggs"
date: "05 JAN 2021"
output:
  html_document:
...
### CCQ Hostile Parenting Adjusting for ASD {.tabset .tabset-fade .tabset-pills}
...
desctable <- data %>%
  ...
  mutate(score1 = case_when( 
    pre_ccqc_mean == -9 ~ NA_real_,          #assign missing values to zeros
    TRUE ~ pre_ccqc_mean)) %>%
  mutate(score2 = case_when( 
    post_ccqc_mean == -9 ~ NA_real_,         #assign missing values to zeros
    TRUE ~ post_ccqc_mean)) %>%
  mutate(score3 = case_when( 
    fu_ccqc_mean == -9 ~ NA_real_,           #assign missing values to zeros
    TRUE ~ fu_ccqc_mean)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(pre_ccqc_mean, na.rm = T ),
            sd1=sd(pre_ccqc_mean, na.rm = T ),
            median1=median(pre_ccqc_mean, na.rm = T ),
            min1=min(pre_ccqc_mean, na.rm = T ),
            max1=max(pre_ccqc_mean, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(post_ccqc_mean, na.rm = T ),
            sd2=sd(post_ccqc_mean, na.rm = T ),
            median2=median(post_ccqc_mean, na.rm = T ),
            min2=min(post_ccqc_mean, na.rm = T ),
            max2=max(post_ccqc_mean, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(fu_ccqc_mean, na.rm = T ),
            sd3=sd(fu_ccqc_mean, na.rm = T ),
            median3=median(fu_ccqc_mean, na.rm = T ),
            min3=min(fu_ccqc_mean, na.rm = T ),
            max3=max(fu_ccqc_mean, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group, pre_ccqc_mean, post_ccqc_mean, fu_ccqc_mean) %>%
  pivot_longer( cols = c("pre_ccqc_mean", "post_ccqc_mean", "fu_ccqc_mean"),  names_prefix = "ccqc_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ccqc_mean",  1,
                ifelse(t == "post_ccqc_mean", 2, 3))) 
...
#  analysis
dat <- data %>%
  dplyr::select(id,group, asdcase, pre_ccqc_mean, post_ccqc_mean, fu_ccqc_mean) %>%
  pivot_longer( cols = c("pre_ccqc_mean", "post_ccqc_mean", "fu_ccqc_mean"),  names_prefix = "ccqc_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ccqc_mean",  1,
                ifelse(t == "post_ccqc_mean", 2, 3))) 
...
The ordering is not as expected.
