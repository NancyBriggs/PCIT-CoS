---
title: "PCIT-CoS-Waitlist RCT Paper 1: ASQ Recoded"
author: "Nancy Briggs"
date: "`r Sys.Date()`"
output:
  html_document:
...
### ASQ {.tabset .tabset-fade .tabset-pills}

Output run on `r Sys.Date()`

dataset: RTC_AQSdata_1_03_2024.xlsx

Variables: t1_aqs_score t2_aqs_score t3_aqs_score
...
data <- data %>%
  dplyr::select(id:treatment_time_mins, dobchild, sexchild, numbhousehold, incfamily,
                ...
                #asq
                asq_t1, asq_t2, asq_t3
                )

data<- data %>%
  mutate(asq_t1=ifelse(asq_t1==999,NA,asq_t1)) %>%
  mutate(asq_t2=ifelse(asq_t2==999,NA,asq_t2)) %>%
  mutate(asq_t3=ifelse(asq_t3==999,NA,asq_t3)) 


# Recoded ASQ
asq <- read_excel("../RTC_AQSdata_1_03_2024.xlsx", 
    sheet = "AQS final summary file")
names(asq) <- tolower(names(asq))
asq <- asq %>%
  rename(id=participant_id) %>%
  rename(asqr_t1=t1_aqs_score) %>%
  rename(asqr_t2=t2_aqs_score) %>%
  rename(asqr_t3=t3_aqs_score) %>%
  mutate(asqr_t1=ifelse(asqr_t1==999,NA,asqr_t1)) %>%
  mutate(asqr_t2=ifelse(asqr_t2==999,NA,asqr_t2)) %>%
  mutate(asqr_t3=ifelse(asqr_t3==999,NA,asqr_t3)) 

data <- left_join(data,asq, by="id")
```
...
desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(asqr_t1)) , 
            mean1=mean(asqr_t1, na.rm = T ),
            sd1=sd(asqr_t1, na.rm = T ),
            median1=median(asqr_t1, na.rm = T ),
            min1=min(asqr_t1, na.rm = T ),
            max1=max(asqr_t1, na.rm = T ),
            
            n2 = sum(!is.na(asqr_t2)) , 
            mean2=mean(asqr_t2, na.rm = T ),
            sd2=sd(asqr_t2, na.rm = T ),
            median2=median(asqr_t2, na.rm = T ),
            min2=min(asqr_t2, na.rm = T ),
            max2=max(asqr_t2, na.rm = T ),
            
            n3 = sum(!is.na(asqr_t3)) , 
            mean3=mean(asqr_t3, na.rm = T ),
            sd3=sd(asqr_t3, na.rm = T ),
            median3=median(asqr_t3, na.rm = T ),
            min3=min(asqr_t3, na.rm = T ),
            max3=max(asqr_t3, na.rm = T )  ) %>%
  pivot_longer(cols = -group,  
                names_to = c('.value', 'time'), 
                    names_pattern = '(.*)(\\d+)')

write.csv(desctable,"desctable_asqr.csv", row.names = TRUE)
...
# plot
p <- data %>%
  dplyr::select(id,group, asqr_t1, asqr_t2, asqr_t3) %>%
  pivot_longer( cols = c("asqr_t1", "asqr_t2", "asqr_t3"),  names_prefix = "ders_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "asqr_t1",  1,
                ifelse(t == "asqr_t2", 2, 3))) 
...
dat <- data %>%
  dplyr::select(id,group,asqr_t1, asqr_t2, asqr_t3) %>%
  pivot_longer( cols = c("asqr_t1", "asqr_t2", "asqr_t3"),  names_prefix = "bitseaprob_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "asqr_t1",  1,
                ifelse(t == "asqr_t2", 2, 3)))
