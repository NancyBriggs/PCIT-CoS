---
title: "PCIT-CoS-Waitlist RCT Paper 1: ASQ"
author: "Nancy Briggs"
date: "`r Sys.Date()`"
output:
  html_document:
...
Variables: ASQ_T1, ASQ_T2, ASQ_T3 
...
desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(asq_t1)) , 
            mean1=mean(asq_t1, na.rm = T ),
            sd1=sd(asq_t1, na.rm = T ),
            median1=median(asq_t1, na.rm = T ),
            min1=min(asq_t1, na.rm = T ),
            max1=max(asq_t1, na.rm = T ),
            
            n2 = sum(!is.na(asq_t2)) , 
            mean2=mean(asq_t2, na.rm = T ),
            sd2=sd(asq_t2, na.rm = T ),
            median2=median(asq_t2, na.rm = T ),
            min2=min(asq_t2, na.rm = T ),
            max2=max(asq_t2, na.rm = T ),
            
            n3 = sum(!is.na(asq_t3)) , 
            mean3=mean(asq_t3, na.rm = T ),
            sd3=sd(asq_t3, na.rm = T ),
            median3=median(asq_t3, na.rm = T ),
            min3=min(asq_t3, na.rm = T ),
            max3=max(asq_t3, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group, asq_t1, asq_t2, asq_t3) %>%
  pivot_longer( cols = c("asq_t1", "asq_t2", "asq_t3"),  names_prefix = "ders_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "asq_t1",  1,
                ifelse(t == "asq_t2", 2, 3))) 
...
dat <- data %>%
  dplyr::select(id,group,asq_t1, asq_t2, asq_t3) %>%
  pivot_longer( cols = c("asq_t1", "asq_t2", "asq_t3"),  names_prefix = "bitseaprob_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "asq_t1",  1,
                ifelse(t == "asq_t2", 2, 3)))
