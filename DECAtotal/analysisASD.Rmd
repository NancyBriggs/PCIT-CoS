---
title: "PCIT-CoS-Waitlist RCT Paper 1: DECA Total Protective Factors ASD"
author: "Nancy Briggs"
date: "05 JAN 2021"
output:
  html_document:
...
### DECA Total Protective Factors Adjusting for ASD {.tabset .tabset-fade .tabset-pills}
...
#  analysis
dat <- data %>%
  dplyr::select(id,group, asdcase, total_1, total_2, total_3) %>%
  pivot_longer( cols = starts_with("total_"), names_prefix = "total_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "1",  1,
                ifelse(t == "2", 2, 3))) 
...
There is no evidence that ASD moderates the 2-way group*time interaction the 3-way interaction (p=0.153). 
...
PCITT incresaed while COSP decreased.  Order is not as hypothesized.
