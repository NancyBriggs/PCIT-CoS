---
title: "PCIT-CoS-Waitlist RCT Paper 1: DECA Total Protective Factors"
author: "Nancy Briggs"
date: "24 May 2022"
output:
  html_document:
...
### DECATotal Protective Factors {.tabset .tabset-fade .tabset-pills}


NOTE THE RESIDUALS FOR THIS OUTCOME ARE NOT GREAT AND A LOG TRANSFORM DOESNT HELP
...
data$time1_deca_protective_tscore_combined <- as.numeric(data$time1_deca_protective_tscore_combined )
data$time2_deca_total_protective_tscore_combined <- as.numeric(data$time2_deca_total_protective_tscore_combined )
data$time3_deca_t_total_protective_tscore_combined <- as.numeric(data$time3_deca_t_total_protective_tscore_combined )

data <- data %>%
  rename(total_1=time1_deca_protective_tscore_combined) %>%
  rename(total_2=time2_deca_total_protective_tscore_combined) %>%
  rename(total_3=time3_deca_t_total_protective_tscore_combined)
```
...
```{r, echo=FALSE}
                ...
                # time2_deca_total_protective_tscore_combined,
                # time3_deca_t_total_protective_tscore_combined
                # all renamed to total_

desctable <- data %>%
  group_by(group) %>%
  mutate(score1 = case_when( 
    total_1 == -9 ~ NA_real_,          #assign missing values to zeros
    TRUE ~ total_1)) %>%
  mutate(score2 = case_when( 
    total_2 == -9 ~ NA_real_,         #assign missing values to zeros
    TRUE ~ total_2)) %>%
  mutate(score3 = case_when( 
    total_3 == -9 ~ NA_real_,           #assign missing values to zeros
    TRUE ~ total_3)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(total_1, na.rm = T ),
            sd1=sd(total_1, na.rm = T ),
            median1=median(total_1, na.rm = T ),
            min1=min(total_1, na.rm = T ),
            max1=max(total_1, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(total_2, na.rm = T ),
            sd2=sd(total_2, na.rm = T ),
            median2=median(total_2, na.rm = T ),
            min2=min(total_2, na.rm = T ),
            max2=max(total_2, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(total_3, na.rm = T ),
            sd3=sd(total_3, na.rm = T ),
            median3=median(total_3, na.rm = T ),
            min3=min(total_3, na.rm = T ),
            max3=max(total_3, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group, total_1, total_2, total_3) %>%
  pivot_longer( cols = starts_with("total_"), names_prefix = "total_", 
...
hist<-ggplot(p, aes(x=outcome) )+
  ...
  xlab("DECA Total Protective Factors") +
  ylab("Frequency") +
...
p<0.05, and the ordering of the change is as predicted (assuming a smaller decrease from T1 to T2 is desired.)
