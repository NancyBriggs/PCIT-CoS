---
title: "PCIT-CoS-Waitlist RCT Paper 1: Child Negativity (NICHD)"
author: "Nancy Briggs"
date: "06 June 2022"
output:
  html_document:
...
### Child Negativity (NICHD) {.tabset .tabset-fade .tabset-pills}
...
# %>%
#   mutate(nichd_child_negative_total_reversed=log(17-nichd_child_negative_total_reversed)) %>%
#   mutate(post_nichd_child_negative_total_reversed=log(17-post_nichd_child_negative_total_reversed)) %>%
#   mutate(fu_nichd_child_negative_total_reversed=log(17-fu_nichd_child_negative_total_reversed) )
...
data$fu_nichd_child_negative_total_reversed[data$group==3] <- NA

desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(nichd_child_negative_total_reversed)) , 
            mean1=mean(nichd_child_negative_total_reversed, na.rm = T ),
            sd1=sd(nichd_child_negative_total_reversed, na.rm = T ),
            median1=median(nichd_child_negative_total_reversed, na.rm = T ),
            min1=min(nichd_child_negative_total_reversed, na.rm = T ),
            max1=max(nichd_child_negative_total_reversed, na.rm = T ),
            
            n2 = sum(!is.na(post_nichd_child_negative_total_reversed)) , 
            mean2=mean(post_nichd_child_negative_total_reversed, na.rm = T ),
            sd2=sd(post_nichd_child_negative_total_reversed, na.rm = T ),
            median2=median(post_nichd_child_negative_total_reversed, na.rm = T ),
            min2=min(post_nichd_child_negative_total_reversed, na.rm = T ),
            max2=max(post_nichd_child_negative_total_reversed, na.rm = T ),
            
            n3 = sum(!is.na(fu_nichd_child_negative_total_reversed)) , 
            mean3=mean(fu_nichd_child_negative_total_reversed, na.rm = T ),
            sd3=sd(fu_nichd_child_negative_total_reversed, na.rm = T ),
            median3=median(fu_nichd_child_negative_total_reversed, na.rm = T ),
            min3=min(fu_nichd_child_negative_total_reversed, na.rm = T ),
            max3=max(fu_nichd_child_negative_total_reversed, na.rm = T )  ) %>%
...
desctable %>%
           ...
           kable_minimal() %>%
           footnote("One participant on the Waitlist has a score at Follow up. It as set to missing.")
```
...
# plot
p <- data %>%
  dplyr::select(id,group,  nichd_child_negative_total_reversed, post_nichd_child_negative_total_reversed, fu_nichd_child_negative_total_reversed) %>%
  pivot_longer( cols = c( "nichd_child_negative_total_reversed", "post_nichd_child_negative_total_reversed", "fu_nichd_child_negative_total_reversed"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "nichd_child_negative_total_reversed",  1,
                ifelse(t == "post_nichd_child_negative_total_reversed", 2, 3))) 
  

  
hist<-ggplot(p, aes(x=outcome) )+
  geom_histogram(alpha=0.6, binwidth = .25) +
  xlab("Child Negativity (NICHD)") +
  ylab("Frequency") +
...
scores did not change for the PCITT group from Baseline to post (-.472), and increased  for COSP (-3.017).
...
pvalue<0.05. We do have evidence that there is a difference in the change over from Baseline to Post amongst the groups.  
...
There is more change in PCITT(-1.381) as in COSP (-.539).
