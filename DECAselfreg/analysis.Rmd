---
title: "PCIT-CoS-Waitlist RCT Paper 1: DECA Self regulation"
author: "Nancy Briggs"
date: "`r Sys.Date()"
output:
  html_document:
...
### DECA Self regulation {.tabset .tabset-fade .tabset-pills}
...
data$time1_deca_t_selfreg_tscore <- as.numeric(data$time1_deca_t_selfreg_tscore )
data$time2_deca_t_selfreg_tscore <- as.numeric(data$time2_deca_t_selfreg_tscore )
data$time3_deca_t_selfreg_tscore <- as.numeric(data$time3_deca_t_selfreg_tscore )

data <- data %>%
  rename(sr_1=time1_deca_t_selfreg_tscore) %>%
  rename(sr_2=time2_deca_t_selfreg_tscore) %>%
  rename(sr_3=time3_deca_t_selfreg_tscore)
```
...
desctable <- data %>%
  ...
  mutate(score1 = case_when( 
    sr_1 == -9 ~ NA_real_,          #assign missing values to zeros
    TRUE ~ sr_1)) %>%
  mutate(score2 = case_when( 
    sr_2 == -9 ~ NA_real_,         #assign missing values to zeros
    TRUE ~ sr_2)) %>%
  mutate(score3 = case_when( 
    sr_3 == -9 ~ NA_real_,           #assign missing values to zeros
    TRUE ~ sr_3)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(sr_1, na.rm = T ),
            sd1=sd(sr_1, na.rm = T ),
            median1=median(sr_1, na.rm = T ),
            min1=min(sr_1, na.rm = T ),
            max1=max(sr_1, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(sr_2, na.rm = T ),
            sd2=sd(sr_2, na.rm = T ),
            median2=median(sr_2, na.rm = T ),
            min2=min(sr_2, na.rm = T ),
            max2=max(sr_2, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(sr_3, na.rm = T ),
            sd3=sd(sr_3, na.rm = T ),
            median3=median(sr_3, na.rm = T ),
            min3=min(sr_3, na.rm = T ),
            max3=max(sr_3, na.rm = T )  ) %>%
...
desctable %>%
           ...
           kable_minimal()
```
 Note this sub scale is only included in the DECA version for children 18 months and older and so there will be missing data/smaller sample size for this analysis
...
# plot
p <- data %>%
  dplyr::select(id,group, sr_1, sr_2, sr_3) %>%
  pivot_longer( cols = starts_with("sr_"), names_prefix = "sr_", 
