...
$H_A: \text {mean change}(\text{PCIT-T}) > \text {mean change}(\text{CoS-P})$ $\&$ 
$\text {mean change}(\text{CoS-P}) > \text {mean change}(\text{Waitlist})$
...
So, the gradient of change is NOT MET.

The p-value below only can be interpreted as a test of whether the change between G1 and G2 & between G2 & G3 is different, but not as supporting the order.
...
p=`r round(s$test$pvalue,4)`. 

<!-- We have `r evidence` that this the change from T1 to T2 is greater for G1 then G2 and the change in G2 is greater than in G3. -->

The p-value only can be interpreted as a test of whether the change between G1 and G2 & between G2 & G3 is different, but not as supporting the order.
