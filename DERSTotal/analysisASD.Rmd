---
title: "PCIT-CoS-Waitlist RCT Paper 1: BITSEA Problem, Controlling for ASD"
author: "Nancy Briggs"
date: "05 JAN 2021"
output:
  html_document:
...
### BITSEA Problem Adjusting for ASD {.tabset .tabset-fade .tabset-pills}
...
desctable <- data %>%
  ...
  mutate(score1 = case_when( 
    pre_bitseaprob_mean == -9 ~ NA_real_,          #assign missing values to zeros
    TRUE ~ pre_bitseaprob_mean)) %>%
  mutate(score2 = case_when( 
    post_bitseaprob_mean == -9 ~ NA_real_,         #assign missing values to zeros
    TRUE ~ post_bitseaprob_mean)) %>%
  mutate(score3 = case_when( 
    fu_bitseaprob_mean == -9 ~ NA_real_,           #assign missing values to zeros
    TRUE ~ fu_bitseaprob_mean)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(pre_bitseaprob_mean, na.rm = T ),
            sd1=sd(pre_bitseaprob_mean, na.rm = T ),
            median1=median(pre_bitseaprob_mean, na.rm = T ),
            min1=min(pre_bitseaprob_mean, na.rm = T ),
            max1=max(pre_bitseaprob_mean, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(post_bitseaprob_mean, na.rm = T ),
            sd2=sd(post_bitseaprob_mean, na.rm = T ),
            median2=median(post_bitseaprob_mean, na.rm = T ),
            min2=min(post_bitseaprob_mean, na.rm = T ),
            max2=max(post_bitseaprob_mean, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(fu_bitseaprob_mean, na.rm = T ),
            sd3=sd(fu_bitseaprob_mean, na.rm = T ),
            median3=median(fu_bitseaprob_mean, na.rm = T ),
            min3=min(fu_bitseaprob_mean, na.rm = T ),
            max3=max(fu_bitseaprob_mean, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,asdcase, group, pre_bitseaprob_mean, post_bitseaprob_mean, fu_bitseaprob_mean) %>%
  pivot_longer( cols = c("pre_bitseaprob_mean", "post_bitseaprob_mean", "fu_bitseaprob_mean"),  names_prefix = "bitseaprob_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_bitseaprob_mean",  1,
                ifelse(t == "post_bitseaprob_mean", 2, 3))) 
...
There is no evidence that ASD moderates the 2-way group*time interaction (the 3-way interaction p=0.683). 
