---
title: "PCIT-CoS-Waitlist RCT Paper 1: Child positivity (NICHD)"
author: "Nancy Briggs"
date: "06 June 2022"
output:
  html_document:
...
### Child positivity (NICHD) {.tabset .tabset-fade .tabset-pills}
...
data <- data %>%
  dplyr::select(id:treatment_time_mins, dobchild, sexchild, numbhousehold, incfamily,
                ...
                nichd_child_positive_total, post_nichd_child_positive_total, fu_nichd_child_positive_total,
...
data$fu_nichd_child_positive_total[data$group==3] <- NA

desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(nichd_child_positive_total)) , 
            mean1=mean(nichd_child_positive_total, na.rm = T ),
            sd1=sd(nichd_child_positive_total, na.rm = T ),
            median1=median(nichd_child_positive_total, na.rm = T ),
            min1=min(nichd_child_positive_total, na.rm = T ),
            max1=max(nichd_child_positive_total, na.rm = T ),
            
            n2 = sum(!is.na(post_nichd_child_positive_total)) , 
            mean2=mean(post_nichd_child_positive_total, na.rm = T ),
            sd2=sd(post_nichd_child_positive_total, na.rm = T ),
            median2=median(post_nichd_child_positive_total, na.rm = T ),
            min2=min(post_nichd_child_positive_total, na.rm = T ),
            max2=max(post_nichd_child_positive_total, na.rm = T ),
            
            n3 = sum(!is.na(fu_nichd_child_positive_total)) , 
            mean3=mean(fu_nichd_child_positive_total, na.rm = T ),
            sd3=sd(fu_nichd_child_positive_total, na.rm = T ),
            median3=median(fu_nichd_child_positive_total, na.rm = T ),
            min3=min(fu_nichd_child_positive_total, na.rm = T ),
            max3=max(fu_nichd_child_positive_total, na.rm = T )  ) %>%
...
desctable %>%
           ...
           kable_minimal() %>%
           footnote("One observation in the Waitlist group has data at Follow up. This was set to missing.")
...
# plot
p <- data %>%
  dplyr::select(id,group,  nichd_child_positive_total, post_nichd_child_positive_total, fu_nichd_child_positive_total) %>%
  pivot_longer( cols = c( "nichd_child_positive_total", "post_nichd_child_positive_total", "fu_nichd_child_positive_total"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "nichd_child_positive_total",  1,
                ifelse(t == "post_nichd_child_positive_total", 2, 3))) 
  

  
hist<-ggplot(p, aes(x=outcome) )+
  geom_histogram(alpha=0.6, binwidth = 5) +
  xlab("Child Positivity (NICHD)") +
  ylab("Frequency") +
...
scores did not change for the PCITT group from Baseline to post (0.056), and decreased slightly for COSP (1.176).
...
There is more change in PCITT(-1.381) than in COSP (-.539).
...
p<0.05, so we do evidence to suggest that the change from t2 to t3 in PCITT is greater than the change in COSP.
