---
title: "PCIT-CoS-Waitlist RCT Paper 1: Observed maternal negativity (NICHD)"
author: "Nancy Briggs"
date: "06 June 2022"
output:
  html_document:
...
### Observed maternal negativity (NICHD) {.tabset .tabset-fade .tabset-pills}
...
data$fu_nichd_negative_total[data$group==3] <- NA

desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(nichd_negative_total)) , 
            mean1=mean(nichd_negative_total, na.rm = T ),
            sd1=sd(nichd_negative_total, na.rm = T ),
            median1=median(nichd_negative_total, na.rm = T ),
            min1=min(nichd_negative_total, na.rm = T ),
            max1=max(nichd_negative_total, na.rm = T ),
            
            n2 = sum(!is.na(post_nichd_negative_total)) , 
            mean2=mean(post_nichd_negative_total, na.rm = T ),
            sd2=sd(post_nichd_negative_total, na.rm = T ),
            median2=median(post_nichd_negative_total, na.rm = T ),
            min2=min(post_nichd_negative_total, na.rm = T ),
            max2=max(post_nichd_negative_total, na.rm = T ),
            
            n3 = sum(!is.na(fu_nichd_negative_total)) , 
            mean3=mean(fu_nichd_negative_total, na.rm = T ),
            sd3=sd(fu_nichd_negative_total, na.rm = T ),
            median3=median(fu_nichd_negative_total, na.rm = T ),
            min3=min(fu_nichd_negative_total, na.rm = T ),
            max3=max(fu_nichd_negative_total, na.rm = T )  ) %>%
...
data_ln<- data %>%
  mutate(nichd_negative_total=log(nichd_negative_total))%>%
  mutate(post_nichd_negative_total=log(post_nichd_negative_total))%>%
  mutate(fu_nichd_negative_total=log(fu_nichd_negative_total))
...
write.csv(desctable,"desctable_log.csv", row.names = TRUE)
...
desctable_ln %>%
           kbl(digits=3, caption="Log Transformed Data") %>%
           kable_minimal() %>%
           footnote("One person the Waitlist has a value at Follow Up. It has been set to missing.",
                    "Data are log transofrmed because residuals from an analysis of the raw data violated assumptions in the model. ")
```
...
# plot
p <- data_ln %>%
  dplyr::select(id,group, nichd_negative_total, post_nichd_negative_total, fu_nichd_negative_total) %>%
  pivot_longer( cols = c("nichd_negative_total", "post_nichd_negative_total", "fu_nichd_negative_total"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "nichd_negative_total",  1,
                ifelse(t == "post_nichd_negative_total", 2, 3))) 
  

  
hist<-ggplot(p, aes(x=outcome) )+
  geom_histogram(alpha=0.6, binwidth = .1) +
  xlab("Observed maternal negativity (NICHD) (log)") +
  ylab("Frequency") +
...
scores decreased for the PCITT group from Baseline to post (0.208), and increased for COSP (-.0175).
...
There is not as much change in PCITT(-.055) as in COSP (-.123).
