---
title: "PCIT-CoS-Waitlist RCT Paper 1: DERS Impulse"
author: "Nancy Briggs"
date: "`r Sys.Date()`"
output:
  html_document:
...
### DERS Impulse {.tabset .tabset-fade .tabset-pills}
...
- sum DERSI3, DERSI14, DERSI19,-1*DERSI24, DERSI27, DERSI32
...
data <- data %>%
  dplyr::select(id:treatment_time_mins, dobchild, sexchild, numbhousehold, incfamily,
                ...
                #DERS Total
                pre_ders_impulse, post_ders_impulse, fu_ders_impulse
                )
...
desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(pre_ders_impulse)) , 
            mean1=mean(pre_ders_impulse, na.rm = T ),
            sd1=sd(pre_ders_impulse, na.rm = T ),
            median1=median(pre_ders_impulse, na.rm = T ),
            min1=min(pre_ders_impulse, na.rm = T ),
            max1=max(pre_ders_impulse, na.rm = T ),
            
            n2 = sum(!is.na(post_ders_impulse)) , 
            mean2=mean(post_ders_impulse, na.rm = T ),
            sd2=sd(post_ders_impulse, na.rm = T ),
            median2=median(post_ders_impulse, na.rm = T ),
            min2=min(post_ders_impulse, na.rm = T ),
            max2=max(post_ders_impulse, na.rm = T ),
            
            n3 = sum(!is.na(fu_ders_impulse)) , 
            mean3=mean(fu_ders_impulse, na.rm = T ),
            sd3=sd(fu_ders_impulse, na.rm = T ),
            median3=median(fu_ders_impulse, na.rm = T ),
            min3=min(fu_ders_impulse, na.rm = T ),
            max3=max(fu_ders_impulse, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group, pre_ders_impulse, post_ders_impulse, fu_ders_impulse) %>%
  pivot_longer( cols = c("pre_ders_impulse", "post_ders_impulse", "fu_ders_impulse"),  names_prefix = "ders_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ders_impulse",  1,
                ifelse(t == "post_ders_impulse", 2, 3))) 
...
dat <- data %>%
  dplyr::select(id,group,pre_ders_impulse, post_ders_impulse, fu_ders_impulse) %>%
  pivot_longer( cols = c("pre_ders_impulse", "post_ders_impulse", "fu_ders_impulse"),  names_prefix = "bitseaprob_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ders_impulse",  1,
                ifelse(t == "post_ders_impulse", 2, 3)))
