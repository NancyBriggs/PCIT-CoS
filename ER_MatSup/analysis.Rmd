---
title: "PCIT-CoS-Waitlist RCT Paper 1: Maternal Support Seeking"
author: "Nancy Briggs"
date: "05 JAN 2021"
output:
  html_document:
...
### Maternal Support Seeking {.tabset .tabset-fade .tabset-pills}
...
data <- data %>%
  dplyr::select(id:treatment_time_mins, dobchild, sexchild, numbhousehold, incfamily,
                ...
                pre_er_matsup_1to5,  post_er_matsup_1to5, fu_er_matsup_1to5
...
desctable <- data %>%
  ...
  mutate(score1 = case_when(
    pre_er_matsup_1to5 == -9 ~ NA_real_,          #assign missing values to zeros
    TRUE ~ pre_er_matsup_1to5)) %>%
  mutate(score2 = case_when(
    post_er_matsup_1to5 == -9 ~ NA_real_,         #assign missing values to zeros
    TRUE ~ post_er_matsup_1to5)) %>%
  mutate(score3 = case_when(
    fu_er_matsup_1to5 == -9 ~ NA_real_,           #assign missing values to zeros
    TRUE ~ fu_er_matsup_1to5)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(pre_er_matsup_1to5, na.rm = T ),
            sd1=sd(pre_er_matsup_1to5, na.rm = T ),
            median1=median(pre_er_matsup_1to5, na.rm = T ),
            min1=min(pre_er_matsup_1to5, na.rm = T ),
            max1=max(pre_er_matsup_1to5, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(post_er_matsup_1to5, na.rm = T ),
            sd2=sd(post_er_matsup_1to5, na.rm = T ),
            median2=median(post_er_matsup_1to5, na.rm = T ),
            min2=min(post_er_matsup_1to5, na.rm = T ),
            max2=max(post_er_matsup_1to5, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(fu_er_matsup_1to5, na.rm = T ),
            sd3=sd(fu_er_matsup_1to5, na.rm = T ),
            median3=median(fu_er_matsup_1to5, na.rm = T )
            # ,
...
I am not sure it is worth analysing these data.  The only group with values at T3 is PCITT.  I recommend not going further. 
...
PCITT Change=-1.23 > COSP change=-0.25 < Waitlist change=-1.07

So, the mean change gradient does not hold, and the pvalue>0.05. We fail to reject Hypothesis 1.
...
There is greater change in PCITT( 4.57) than in COSP (-2.67) (But I don't know if this indicates more benefit in PCITT or not)
...
p<0.05, so we do  evidence to suggest that the change from t2 to t3 in PCITT is different than the change in COSP.
...
p<0.05. So there is evidence that the change from t1 to t3 is different for G1 and G2.  Note that the mean difference between the two groups at T3 is NOT significant.
