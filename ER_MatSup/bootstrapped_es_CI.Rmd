...
s_groupdiffs %>%
  dplyr::select(c(-comparison,-df, -es2_5, -es97_5)) %>%
  kbl(digits=3,caption = "Group Differences at Each Time Point", 
  col.names = c("Group Difference", "Time","Mean", "SE", "t","p","adj p",
                  "Lower CL","Upper CL", "Effect Size") )  %>%
  kable_styling() %>%
  add_footnote( c("Effect size scaled by pooled T1 SD", "No bootstrapping performed"), notation = "symbol")
```
...
s %>%
  dplyr::select(c(-comparison,-df, -esch2_5, -esch97_5)) %>%
