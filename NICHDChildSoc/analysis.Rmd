---
title: "PCIT-CoS-Waitlist RCT Paper 1: Child Sociability (NICHD)"
author: "Nancy Briggs"
date: "06 June 2022"
output:
  html_document:
...
### Child Sociability (NICHD {.tabset .tabset-fade .tabset-pills}
...
data$fu_nichd_child_sociability_total[data$group==3] <- NA

desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(nichd_child_sociability_total)) , 
            mean1=mean(nichd_child_sociability_total, na.rm = T ),
            sd1=sd(nichd_child_sociability_total, na.rm = T ),
            median1=median(nichd_child_sociability_total, na.rm = T ),
            min1=min(nichd_child_sociability_total, na.rm = T ),
            max1=max(nichd_child_sociability_total, na.rm = T ),
            
            n2 = sum(!is.na(post_nichd_child_sociability_total)) , 
            mean2=mean(post_nichd_child_sociability_total, na.rm = T ),
            sd2=sd(post_nichd_child_sociability_total, na.rm = T ),
            median2=median(post_nichd_child_sociability_total, na.rm = T ),
            min2=min(post_nichd_child_sociability_total, na.rm = T ),
            max2=max(post_nichd_child_sociability_total, na.rm = T ),
            
            n3 = sum(!is.na(fu_nichd_child_sociability_total)) , 
            mean3=mean(fu_nichd_child_sociability_total, na.rm = T ),
            sd3=sd(fu_nichd_child_sociability_total, na.rm = T ),
            median3=median(fu_nichd_child_sociability_total, na.rm = T ),
            min3=min(fu_nichd_child_sociability_total, na.rm = T ),
            max3=max(fu_nichd_child_sociability_total, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group,   nichd_child_sociability_total, post_nichd_child_sociability_total, fu_nichd_child_sociability_total) %>%
  pivot_longer( cols = c(  "nichd_child_sociability_total", "post_nichd_child_sociability_total", "fu_nichd_child_sociability_total"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "nichd_child_sociability_total",  1,
                ifelse(t == "post_nichd_child_sociability_total", 2, 3))) 
  

  
hist<-ggplot(p, aes(x=outcome) )+
  geom_histogram(alpha=0.6, binwidth = 1) +
  xlab("Observed Chils Sociability (NICHD)") +
  ylab("Frequency") +
...
scores increased for the PCITT group from Baseline to post (-1.884), and increased for COSP (-2.458).
...
pvalue>0.05. We do not have evidence that there is a difference in the change over from Baseline to Post amongst the groups. 
...
There is not as much change in PCITT(.295) as in COSP (1.767).
