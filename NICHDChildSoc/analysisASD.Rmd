---
title: "PCIT-CoS-Waitlist RCT Paper 1: Child Sociability (NICHD), Controlling for ASD"
author: "Nancy Briggs"
date: "05 JAN 2021"
output:
  html_document:
...
###  Child Sociability (NICHD) Adjusting for ASD {.tabset .tabset-fade .tabset-pills}
...
#  analysis
dat <- data %>%
  dplyr::select(id,group, asdcase,  nichd_child_sociability_total, post_nichd_child_sociability_total, fu_nichd_child_sociability_total) %>%
  pivot_longer( cols = c(  "nichd_child_sociability_total", "post_nichd_child_sociability_total", "fu_nichd_child_sociability_total"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "nichd_child_sociability_total",  1,
                ifelse(t == "post_nichd_child_sociability_total", 2, 3))) 
...
There is no evidence that ASD moderates the 2-way group*time interaction (the 3-way interaction p=0.887). 
...
pvalue>0.05. We have no evidence that there is a difference in the change over from Baseline to Post amongst the groups, such that PCITT showed the largest change, then COSP then Waitlist.
