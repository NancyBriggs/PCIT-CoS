---
title: "PCIT-CoS-Waitlist RCT Paper 1: Parental Reflectiveness CCQE, Controlling for ASD"
author: "Nancy Briggs"
date: "05 JAN 2021"
output:
  html_document:
...
###  Parental Reflectiveness CCQE Adjusting for ASD {.tabset .tabset-fade .tabset-pills}
...
data <- data %>%
  dplyr::select(id:treatment_time_mins, dobchild, sexchild, numbhousehold, incfamily,
               ...
               # Maternal reflective functioning 
                pre_ccqe_mean, post_ccqe_mean, fu_ccqe_mean,
...
desctable <- data %>%
  ...
  summarise(n1 = sum(!is.na(pre_ccqe_mean)) , 
            mean1=mean(pre_ccqe_mean, na.rm = T ),
            sd1=sd(pre_ccqe_mean, na.rm = T ),
            median1=median(pre_ccqe_mean, na.rm = T ),
            min1=min(pre_ccqe_mean, na.rm = T ),
            max1=max(pre_ccqe_mean, na.rm = T ),
            
            n2 = sum(!is.na(post_ccqe_mean)) , 
            mean2=mean(post_ccqe_mean, na.rm = T ),
            sd2=sd(post_ccqe_mean, na.rm = T ),
            median2=median(post_ccqe_mean, na.rm = T ),
            min2=min(post_ccqe_mean, na.rm = T ),
            max2=max(post_ccqe_mean, na.rm = T ),
            
            n3 = sum(!is.na(fu_ccqe_mean)) , 
            mean3=mean(fu_ccqe_mean, na.rm = T ),
            sd3=sd(fu_ccqe_mean, na.rm = T ),
            median3=median(fu_ccqe_mean, na.rm = T ),
            min3=min(fu_ccqe_mean, na.rm = T ),
            max3=max(fu_ccqe_mean, na.rm = T )  ) %>%
...
data_ln<- data %>%
  mutate(pre_ccqe_mean=log(pre_ccqe_mean))%>%
  mutate(post_ccqe_mean=log(post_ccqe_mean))%>%
  mutate(fu_ccqe_mean=log(fu_ccqe_mean))
desctable_ln <- data_ln %>%
  group_by(asdcase, group) %>%
  summarise(n1 = sum(!is.na(pre_ccqe_mean)) , 
            mean1=mean(pre_ccqe_mean, na.rm = T ),
            sd1=sd(pre_ccqe_mean, na.rm = T ),
...
write.csv(desctable,"desctable_asd_ln.csv", row.names = TRUE)
...
# plot
p <- data_ln %>%
  dplyr::select(id,group, pre_ccqe_mean, post_ccqe_mean, fu_ccqe_mean) %>%
  pivot_longer( cols = c("pre_ccqe_mean", "post_ccqe_mean", "fu_ccqe_mean"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ccqe_mean",  1,
                ifelse(t == "post_ccqe_mean", 2, 3))) 
...
hist<-ggplot(p, aes(x=outcome) )+
  ...
  xlab("Parental Reflexiveness CCQE") +
  ylab("Frequency") +
...
#  analysis
dat <- data_ln %>%
  dplyr::select(id,group, asdcase, pre_ccqe_mean, post_ccqe_mean, fu_ccqe_mean) %>%
  pivot_longer( cols = c("pre_ccqe_mean", "post_ccqe_mean", "fu_ccqe_mean"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ccqe_mean",  1,
                ifelse(t == "post_ccqe_mean", 2, 3))) 
...
Interesting! there may be an effect of ASD on the group* time interaction.

Ok, we have not had this hapen before.  I think what I will do is proceed as previous, using the model with group*time and adjusting for the main effect of ASD case and present those results.  Then, I will explore how the pattern of change over time for the 2 ASD groups differs.
...
```{r, echo=FALSE}
emmip(result3, group ~ time, CIs = TRUE)
...
PCITT increases from baseline to post (-.120) while there is minimal change in COSP and Waitlist.
...
#### ASD Case Interaction

In the above model, the interaction between ASD, group and time was significant, indicating that the pattern of change over time is different for those with and without an ASD diagnosis.

Now, the thing to be careful with here is that there are relatively few ASD cases, and by the post time point, there are only 2 in the COSP group.
...
Here is what the pattern of means looks like:

```{r, echo=FALSE}
emmip(result4, group ~ time |asdcase, CIs = TRUE)
```

First, we can see that the CIs for the ASD group are much wider, which makes sense because there are so few kids in that group.

Next, we can see that the patterns for the PCITT and Waitlist groups are similar; it's really the COSP group (green) that is quite different.  My guess is it is this difference that is driving the p-value for the 3-way interaction. Given that the there are only 2 people in the COSP group at Post, I would not put too much faith in this as an effect that would remain if there were a larger sample.
