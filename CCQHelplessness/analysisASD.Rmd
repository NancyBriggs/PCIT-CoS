...
### CCQ Caregiving Helplessness Adjusting for ASD {.tabset .tabset-fade .tabset-pills}
...
p <- data %>%
  ...
  pivot_longer( cols = c("pre_chqd_mean", "post_chqd_mean", "fu_chqd_mean"),  names_prefix = "ccqa_", 
...
#  analysis
dat <- data %>%
  dplyr::select(id,group, asdcase, pre_chqd_mean, post_chqd_mean, fu_chqd_mean) %>%
  pivot_longer( cols = c("pre_chqd_mean", "post_chqd_mean", "fu_chqd_mean"),  names_prefix = "ccqa_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_chqd_mean",  1,
                ifelse(t == "post_chqd_mean", 2, 3))) 
