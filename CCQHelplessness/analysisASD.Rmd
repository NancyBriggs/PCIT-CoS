---
title: "PCIT-CoS-Waitlist RCT Paper 1:CCQ Caregiving Helplessness, Controlling for ASD"
author: "Nancy Briggs"
date: "r Sys.Date()"
output:
  html_document:
...
p <- data %>%
  ...
  pivot_longer( cols = c("pre_chqd_sum", "post_chqd_sum", "fu_chqd_sum"),  names_prefix = "ccqa_", 
...
#  analysis
dat <- data %>%
  dplyr::select(id,group, asdcase, pre_chqd_sum, post_chqd_sum, fu_chqd_sum) %>%
  pivot_longer( cols = c("pre_chqd_sum", "post_chqd_sum", "fu_chqd_sum"),  names_prefix = "ccqa_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_chqd_sum",  1,
                ifelse(t == "post_chqd_sum", 2, 3))) 
...
evidence = ifelse(anovatab[6,9]<0.05, "evidence", "no evidence")
```

There is no `r evidence` that ASD moderates the 2-way group*time interaction (the 3-way interaction p=`r round(anovatab[6,9],4)` ). 
