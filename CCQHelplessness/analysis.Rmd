...
### CCQ Caregiving Helplessness  {.tabset .tabset-fade .tabset-pills}

Output run on `r Sys.Date()`


Values are totals, not means, of the following CCQ variables: CCQD1, CCQD2, CCQD3, CCQD4, CCQD5, CCQD6, CCQD7
...
desctable <- data %>%
  ...
  mutate(score1 = case_when( 
    pre_chqd_sum == -9 ~ NA_real_,          #assign missing values to zeros
    TRUE ~ pre_chqd_sum)) %>%
  mutate(score2 = case_when( 
    post_chqd_sum == -9 ~ NA_real_,         #assign missing values to zeros
    TRUE ~ post_chqd_sum)) %>%
  mutate(score3 = case_when( 
    fu_chqd_sum == -9 ~ NA_real_,           #assign missing values to zeros
    TRUE ~ fu_chqd_sum)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(pre_chqd_sum, na.rm = T ),
            sd1=sd(pre_chqd_sum, na.rm = T ),
            median1=median(pre_chqd_sum, na.rm = T ),
            min1=min(pre_chqd_sum, na.rm = T ),
            max1=max(pre_chqd_sum, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(post_chqd_sum, na.rm = T ),
            sd2=sd(post_chqd_sum, na.rm = T ),
            median2=median(post_chqd_sum, na.rm = T ),
            min2=min(post_chqd_sum, na.rm = T ),
            max2=max(post_chqd_sum, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(fu_chqd_sum, na.rm = T ),
            sd3=sd(fu_chqd_sum, na.rm = T ),
            median3=median(fu_chqd_sum, na.rm = T ),
            min3=min(fu_chqd_sum, na.rm = T ),
            max3=max(fu_chqd_sum, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group, pre_chqd_sum, post_chqd_sum ,fu_chqd_sum) %>%
  pivot_longer( cols = c("pre_chqd_sum", "post_chqd_sum", "fu_chqd_sum"),  names_prefix = "chqd_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_chqd_sum",  1,
                ifelse(t == "post_chqd_sum", 2, 3))) 
