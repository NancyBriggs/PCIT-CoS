---
title: "PCIT-CoS-Waitlist RCT Paper 1: TOPSE Empathy"
author: "Nancy Briggs"
date: "`r Sys.Date()`"
output:
  html_document:
...
### TOPSE Empathy {.tabset .tabset-fade .tabset-pills}
...
- sum computed from CCQA1, CCQA2,  CCQA3,  CCQA4, CCQA5,  CCQA6
...
data <- data %>%
  dplyr::select(id:treatment_time_mins, dobchild, sexchild, numbhousehold, incfamily,
                ...
                #TOPSE Empathy 
                pre_topse_empathy_sum, post_topse_empathy_sum, fu_topse_empathy_sum
                )
...
desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(pre_topse_empathy_sum)) , 
            mean1=mean(pre_topse_empathy_sum, na.rm = T ),
            sd1=sd(pre_topse_empathy_sum, na.rm = T ),
            median1=median(pre_topse_empathy_sum, na.rm = T ),
            min1=min(pre_topse_empathy_sum, na.rm = T ),
            max1=max(pre_topse_empathy_sum, na.rm = T ),
            
            n2 = sum(!is.na(post_topse_empathy_sum)) , 
            mean2=mean(post_topse_empathy_sum, na.rm = T ),
            sd2=sd(post_topse_empathy_sum, na.rm = T ),
            median2=median(post_topse_empathy_sum, na.rm = T ),
            min2=min(post_topse_empathy_sum, na.rm = T ),
            max2=max(post_topse_empathy_sum, na.rm = T ),
            
            n3 = sum(!is.na(fu_topse_empathy_sum)) , 
            mean3=mean(fu_topse_empathy_sum, na.rm = T ),
            sd3=sd(fu_topse_empathy_sum, na.rm = T ),
            median3=median(fu_topse_empathy_sum, na.rm = T ),
            min3=min(fu_topse_empathy_sum, na.rm = T ),
            max3=max(fu_topse_empathy_sum, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group, pre_topse_empathy_sum, post_topse_empathy_sum, fu_topse_empathy_sum) %>%
  pivot_longer( cols = c("pre_topse_empathy_sum", "post_topse_empathy_sum", "fu_topse_empathy_sum"),  names_prefix = "ders_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_topse_empathy_sum",  1,
                ifelse(t == "post_topse_empathy_sum", 2, 3))) 
...
dat <- data %>%
  dplyr::select(id,group,pre_topse_empathy_sum, post_topse_empathy_sum, fu_topse_empathy_sum) %>%
  pivot_longer( cols = c("pre_topse_empathy_sum", "post_topse_empathy_sum", "fu_topse_empathy_sum"),  names_prefix = "bitseaprob_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_topse_empathy_sum",  1,
                ifelse(t == "post_topse_empathy_sum", 2, 3)))
