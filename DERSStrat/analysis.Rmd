---
title: "PCIT-CoS-Waitlist RCT Paper 1: DERS Strategy"
author: "Nancy Briggs"
date: "`r Sys.Date()`"
output:
  html_document:
...
### DERS Strategy {.tabset .tabset-fade .tabset-pills}
...
- sum computed from DERSI15,DERSI16,-1*DERSI22,DERSI28,DERSI30,DERSI31,DERSI35,DERSI36
...
data <- data %>%
  dplyr::select(id:treatment_time_mins, dobchild, sexchild, numbhousehold, incfamily,
                ...
                #DERS Strat
                pre_ders_strat, post_ders_strat, fu_ders_strat
                )
...
desctable <- data %>%
  group_by(group) %>%
  summarise(n1 = sum(!is.na(pre_ders_strat)) , 
            mean1=mean(pre_ders_strat, na.rm = T ),
            sd1=sd(pre_ders_strat, na.rm = T ),
            median1=median(pre_ders_strat, na.rm = T ),
            min1=min(pre_ders_strat, na.rm = T ),
            max1=max(pre_ders_strat, na.rm = T ),
            
            n2 = sum(!is.na(post_ders_strat)) , 
            mean2=mean(post_ders_strat, na.rm = T ),
            sd2=sd(post_ders_strat, na.rm = T ),
            median2=median(post_ders_strat, na.rm = T ),
            min2=min(post_ders_strat, na.rm = T ),
            max2=max(post_ders_strat, na.rm = T ),
            
            n3 = sum(!is.na(fu_ders_strat)) , 
            mean3=mean(fu_ders_strat, na.rm = T ),
            sd3=sd(fu_ders_strat, na.rm = T ),
            median3=median(fu_ders_strat, na.rm = T ),
            min3=min(fu_ders_strat, na.rm = T ),
            max3=max(fu_ders_strat, na.rm = T )  ) %>%
...
# plot
p <- data %>%
  dplyr::select(id,group, pre_ders_strat, post_ders_strat, fu_ders_strat) %>%
  pivot_longer( cols = c("pre_ders_strat", "post_ders_strat", "fu_ders_strat"),  names_prefix = "ders_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ders_strat",  1,
                ifelse(t == "post_ders_strat", 2, 3))) 
...
dat <- data %>%
  dplyr::select(id,group,pre_ders_strat, post_ders_strat, fu_ders_strat) %>%
  pivot_longer( cols = c("pre_ders_strat", "post_ders_strat", "fu_ders_strat"),  names_prefix = "bitseaprob_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "pre_ders_strat",  1,
                ifelse(t == "post_ders_strat", 2, 3)))
