---
title: "PCIT-CoS-Waitlist RCT Paper 1: DPICS LP, Controlling for ASD"
author: "Nancy Briggs"
date: "31 JAN 2021"
output:
  html_document:
...
### DPICS LP Controlling for ASD {.tabset .tabset-fade .tabset-pills}
...
Note that with the number of ASD cases, it is unliekly that a model with the group*time *ASD interction will run at all. 
...
result0_5<-glmer(cbind(lp, verbal-lp) ~ asdcase + (1 | id), data=p, family=binomial)
result1<-glmer(cbind(lp, verbal-lp) ~ asdcase + group + (1 | id), data=p, family=binomial)
result2<-glmer(cbind(lp, verbal-lp) ~ asdcase + group + time + (1 | id), data=p, family=binomial)
result3<-glmer(cbind(lp, verbal-lp) ~ asdcase + group + time +group*time + (1 | id), data=p, family=binomial)
```

The model with asdcase*group*time does not converge. We will leave it out.

```{r}
result4<-glmer(cbind(lp, verbal-lp) ~ asdcase*group*time + (1 | id), data=p, family=binomial)
```
...
$\frac{1}{0.0374} = 26.7380$

and the CI limits:

$\frac{1}{0.0642} = 15.5763$

$\frac{1}{0.0218} = 45.8716$

Here, the odds of an LP verbalisation are 26 times higher at time 2 than time 1 for PCIT-T.  You should temper this HUGE number with the fact that the proportions of LP actions is very small, regardless of the time and group, however (.14% at time 1, and
3.6% at time 2 for PCIT-T.
...
p>0.05, so this null hypotheiss connote rejected.  However, it is important to note that the prportions at time 3 for PCITT (2.2%) and COSP (0.29%) are very different.
