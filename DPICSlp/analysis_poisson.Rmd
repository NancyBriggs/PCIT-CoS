...
I was not happy with modelling this DPICS variable as a proportion of total verbalisation since all the proportions ended up being so small (total verbalisations were in the hundreds, with LP actions being small, usually <10%).

So I switched to a poisson model, with no offset, simply looking at total number of LP verbalisations, and I am much happier with the output and results.

In the results, the "effect size" the the ratio of the means.
...
desctableTotal <- data %>%
  group_by(group) %>%
  mutate(score1 = case_when( 
    parent_total_lp_time_1 == -9 ~ NA_real_,                                #assign missing values to zeros
    TRUE ~ parent_total_lp_time_1)) %>%
  mutate(score2 = case_when( 
    parent_total_lp_time_2 == -9 ~ NA_real_,                                #assign missing values to zeros
    TRUE ~ parent_total_lp_time_2)) %>%
  mutate(score3 = case_when( 
    parent_total_lp_time_3 == -9 ~ NA_real_,                                #assign missing values to zeros
    TRUE ~ parent_total_lp_time_3)) %>%
  summarise(n1 = sum(!is.na(score1)) , 
            mean1=mean(parent_total_lp_time_1, na.rm = T ),
            sd1=sd(parent_total_lp_time_1, na.rm = T ),
            median1=median(parent_total_lp_time_1, na.rm = T ),
            min1=min(parent_total_lp_time_1, na.rm = T ),
            max1=max(parent_total_lp_time_1, na.rm = T ),
            
            n2 = sum(!is.na(score2)) , 
            mean2=mean(parent_total_lp_time_2, na.rm = T ),
            sd2=sd(parent_total_lp_time_2, na.rm = T ),
            median2=median(parent_total_lp_time_2, na.rm = T ),
            min2=min(parent_total_lp_time_2, na.rm = T ),
            max2=max(parent_total_lp_time_2, na.rm = T ),
            
            n3 = sum(!is.na(score3)) , 
            mean3=mean(parent_total_lp_time_3, na.rm = T ),
            sd3=sd(parent_total_lp_time_3, na.rm = T ),
            median3=median(parent_total_lp_time_3, na.rm = T ),
            min3=min(parent_total_lp_time_3, na.rm = T ),
            max3=max(parent_total_lp_time_3, na.rm = T )  ) %>%
  pivot_longer(cols = -group,  
                names_to = c('.value', 'time'), 
                    names_pattern = '(.*)(\\d+)')

write.csv(desctableTotal,"desctable_numberLP.csv", row.names = TRUE)
...
_Descriptives of Total Number of LP_
...
# plot
p <- data %>%
  dplyr::select(id,group, parent_total_lp_time_1, parent_total_lp_time_2, parent_total_lp_time_3) %>%
  pivot_longer( cols = starts_with("parent_total_lp_time_"), names_prefix = "time", 
...
A generalized linear mixed model (GLMM) with random effect for individual and fixed effects of group, time and the interaction was run on the number of LP responses. A poisson distribution was specified with a log link. Estimated marginal means were obtained, and contrasts specified to test Hypothesis 1 and Hypothesis 2. Assumptions on the residuals were examined using simulated residuals (see below).
...
result0<-glmer(lp ~ (1 | id), data=p, family=poisson)
result1<-glmer(lp ~ group + (1 | id), data=p, family=poisson)
result2<-glmer(lp ~ group + time + (1 | id), data=p, family=poisson)
result3<-glmer(lp ~ group + time +group*time + (1 | id), data=p, family=poisson)
...
dplyr::select(c(-null,-df)) %>%
  ...
  col.names = c("Comparison", "Time","Ratio", "SE", "z","p","adj p",
                  "Lower CL","Upper CL") )  %>%
  kable_styling() %>%
  add_footnote( c("Effect size here is the ratio of differences. A value greater than 1 indicates a proportion change from the reference group." ), notation = "symbol")
```

Here you can see that Group1 has a lower number of LP at time 1 than Group1 (Ratio=.69), but at Time 2, it is much higher (ratio=10.96). 
...
Here, you can see that the effect size for change in group1 from T1 to T2 is 25.4. This can be interpreted as: the number of LP at time 2 was 25.4 times that of LP at time1. That's huge!  The value for group2 is 1.6, showing an increase in 1.6 times LP at time 1, but it is so much smaller than the relative increase for Group1.  
...
lp1 <- ggplot(data=result.means, aes(x=time1, y=rate, group=group, shape=group, colour=group)) + geom_line(aes(linetype=group)) + geom_point(size=2) +
...
so, change from t1 to t2 is represented as an rate ratio.  They way these results are presented, a value greater than 1 means that there is an *increase* from T1 to T2.  
...
p>0.05, so this null hypothesis cannot be rejected.  
