---
title: "PCIT-CoS-Waitlist RCT Paper 1: Observed maternal negativity (NICHD), Controlling for ASD"
author: "Nancy Briggs"
date: "05 JAN 2021"
output:
  html_document:
...
### Observed maternal negativity (NICHD) Adjusting for ASD {.tabset .tabset-fade .tabset-pills}
...
#  analysis
dat <- data_ln %>%
  dplyr::select(id,group, asdcase, nichd_negative_total, post_nichd_negative_total, fu_nichd_negative_total) %>%
  pivot_longer( cols = c("nichd_negative_total", "post_nichd_negative_total", "fu_nichd_negative_total"),  names_prefix = "bitseacomp_", 
                names_to="t", values_to="outcome") %>%
  mutate(time = ifelse(t == "nichd_negative_total",  1,
                ifelse(t == "post_nichd_negative_total", 2, 3))) 
...
There is no evidence that ASD moderates the 2-way group*time interaction (the 3-way interaction p=0.146). 
